...
class GMEEK():
    ...
    def cleanFile(self):
        ...
        cname_path = os.path.join(workspace_path, self.root_dir, 'CNAME')

        # 备份 CNAME 文件（如果存在）
        if os.path.exists(cname_path):
            shutil.copy(cname_path, workspace_path + "/CNAME_backup")
        ...
        # 恢复 CNAME 文件（如果存在）
        if os.path.exists(workspace_path + "/CNAME_backup"):
            shutil.copy(workspace_path + "/CNAME_backup", self.root_dir + "/CNAME")
            os.remove(workspace_path + "/CNAME_backup")
...
if os.environ.get('GITHUB_EVENT_NAME')!='schedule':
    ...
    readme=readme+"### Powered by :heart: [风言锋语88](https://2015888.xyz/wo)\r\n"
